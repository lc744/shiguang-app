...
/* ---------------- 悬浮球拖动 ---------------- */
(function(){
  const POS_KEY = 'shiguang_genie_pos';
  const TAP_LIMIT = 8;            // 位移小于此值视为轻点
  let ball = null;
  let dragging = false;
  let moved = false;
  ...
  function clamp(x, y){
    const w = ball.offsetWidth || 54, h = ball.offsetHeight || 54;
  ...
  function place(x, y, save){
    ...
    if(save){ try{ localStorage.setItem(POS_KEY, JSON.stringify({ x: p.x, y: p.y })); }catch(e){} }
  }
  function restore(){
    ball = document.getElementById('genieFab');
    if(!ball) return;
    let pos = null;
    ...
    if(pos && typeof pos.x === 'number' && typeof pos.y === 'number'){
      ...
      place(window.innerWidth - 54 - 18, window.innerHeight - 150 - 54, false);
    }
  }
  function onDown(e){
  ...
  function onMove(e){
    ...
    if(!moved && Math.abs(dx) + Math.abs(dy) > TAP_LIMIT) moved = true;
    if(moved) place(origX + dx, origY + dy, false);
  }
  function onUp(){
    if(!dragging) return;
    dragging = false;
  ...
  function init(){
    ...
    ball.addEventListener('pointercancel', () => { dragging = false; ball.style.transition = ''; });
    window.__gdrag = () => ({ dragging, moved, hasBall: !!ball });
  }
